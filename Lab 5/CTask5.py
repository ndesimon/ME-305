...
def loopFunction(taskName, period, kFlag, CLC, eulAng, wFlag, KpVal, KdVal, KiVal, RefVal, duty, gyrVel, numEul, numGyr, dFlag):
    '''! @brief Generator function that passes the duty cycles to the motor.
         ...
         @param KdVal A shared parameter that indicates the gain value Kd.
    ...
    while True:
        ...
        if ticks_diff(current_time, next_time) >= 0:
            ...
            if state == S0_INIT:
                ...
                if wFlag.read():
                    controller.set_Gain(KpVal.read(), KdVal.read(), KiVal.read())
                    controller.set_Reference(RefVal.read())
                    act_sig = controller.run(eulAng.read()[numEul], gyrVel.read()[numGyr])
