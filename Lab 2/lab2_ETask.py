...
@details Constantly collects position, time, and delta values from the encoder.
         The state transition diagram for this task is depicted below:
         @image html ETaskSTD.png width=500px
         
         See source code:
         https://bitbucket.org/bbartl01/mechatronics-labs/src/master/Lab2/lab2_ETask.py
...
def updateFunction(taskName, period, zFlag, encData):
    ...
    #
    ENC1 = encoder.Encoder(pyb.Pin.cpu.B6, pyb.Pin.cpu.B7, 4)
    ...
    while True:
        ...
        if ticks_diff(current_time, next_time) >= 0:
            ...
            if state == S0_INIT:
                ...
                ENC1.update()
                encData.write((ticks_ms(), ENC1.get_position(), ENC1.get_delta()))
                if zFlag.read():        
                    state = S2_ZERO
